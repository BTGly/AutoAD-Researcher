...
paragraphs:
  - paragraph_id
    paragraph_kind: background / interpretation / limitation / recommendation
    prose_template
    claim_ids: list[str]
```
...
`paragraph_kind` 是结构字段，不由 Validator 从自由文本关键词猜测：`background` 的 `claim_ids` 可以为空；`interpretation` 和 `limitation` 至少需要一个 `claim_id`；`recommendation` 只有在引用报告事实时才需要 claim。这样保持软约束，不引入逐句事实图谱或 LLM 分类器。
...
paragraph_kind_requirements_hold
...
- [ ] `paragraph_kind` 的结构性要求由 Validator 确定性检查，不能通过改变自然语言或关键词绕过。

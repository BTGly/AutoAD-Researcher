...
### Session 4: 后端框架选型比较文档

**目标**: 将讨论中的 12 个后端框架比较矩阵整理为正式文档 `docs/backend_framework_comparison.md`。

**操作**:

1. 创建 `docs/backend_framework_comparison.md`
   - 从讨论矩阵提取 12 个框架的完整比较（自研 Core、Deep Agents、LangGraph、Pydantic AI、CrewAI、AutoGen、OpenHands、SWE-agent、Google ADK、LlamaIndex、Semantic Kernel、Temporal）
   - 新增第 0 节"选型前提"：AutoAD Core 定位重申 + 分层选型框架图（Core → Harness → Pipeline Runtime → Subsystems）
   - 新增优先级定义表（P0-P3 + 必须保留/低优先级）
   - 重组为 6 大节：综合比较矩阵、关键候选深度分析、推荐组合方案（3 个方案）、决策时间线、官方定位摘要、补充说明
   - 方案 A（Deep Agents 主线）为 MVP 推荐；方案 B（LangGraph 自建）为后期备选；方案 C（Pydantic AI 中心）待验证
   - 补充说明涵盖：过早工程化风险、角色扮演式多 Agent 陷阱、artifact-first 原则

2. 与技术路线草案对齐
   - 引用第一层开发原则（AutoAD Core 是领域闭环控制层，Deep Agents 是可选执行内核）
   - 坚持 artifact-first 原则，避免对话-first 的框架选择偏差

**遗留问题**:
- 需要在 MVP 阶段实际验证 Deep Agents 和 Pydantic AI 的对比结论
- 方案 B（LangGraph 自建）的切换时机需要更量化的判断标准

**下一步**: 继续 Step 2.5 或其他架构开发

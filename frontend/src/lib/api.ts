...
export async function sendReportDiscussion(runId: string, reportId: string, requestId: string, content: string): Promise<DiscussionTurn> { const res = await fetch(`/api/runs/${runId}/reports/${reportId}/discussion`, { method: 'POST', headers: getHeaders(), body: JSON.stringify({ request_id: requestId, content }) }); if (!res.ok) throw await apiError(res, 'Discussion request failed'); const turn = await res.json() as DiscussionTurn; if (turn.status !== 'completed') throw new ApiError(res.status, turn.error || (turn.status === 'pending' ? '报告讨论仍在处理中，请稍后重试。' : '报告讨论失败，请重试。')); return turn; }

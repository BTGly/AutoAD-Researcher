...
export function ExperimentPage({ runId, experimentRefreshTick, onOpenExperimentSettings, onDiscuss }: Props) {
  ...
  return <main className="observatory">
    <header className="observatory-header">
      <div><h1>实验工作台</h1><div>持久化实验状态的只读观测与受限显式审批</div></div>
      <div className="observatory-actions"><AppButton onClick={() => void loadProjection(runId, sessionId)} disabled={!runId || loading}><RefreshCw size={15} aria-hidden="true" />{loading ? '刷新中' : '刷新'}</AppButton><AppButton onClick={onOpenExperimentSettings}><Settings2 size={15} aria-hidden="true" />实验配置</AppButton></div>
    </header>
    {!runId && <EmptyState title="请先创建一个研究任务。" detail="研究任务创建后，实验事实会在这里以只读投影呈现。" />}
    {runId && loading && !projection && <EmptyState title="正在读取实验状态…" />}
    {runId && error && <div role="alert" className="observatory-alert">{error}</div>}
    {runId && projection?.selection_status === 'no_session' && <EmptyState title="实验尚未启动。" detail="请先在研究对话中确认实验任务。" />}
    {runId && projection?.selection_status === 'ambiguous' && <Surface className="observatory-session-picker"><b>发现多个实验 Session，请明确选择</b><select aria-label="实验 Session" value={sessionId || ''} onChange={event => chooseSession(event.target.value)}><option value="">请选择</option>{projection.session_candidates.map(item => <option key={item.session_id} value={item.session_id}>{item.session_id} · {sessionStatusLabel(item.status)}</option>)}</select></Surface>}
    {runId && projection?.selection_status === 'selected' && projection.session && projection.summary && <>
      ...
      <div className="observatory-layout">
        <aside className="observatory-outline"><Panel title="Idea Tree"><IdeaTree nodes={projection.idea_tree?.nodes || []} championIdeaId={projection.champion?.idea_id || null} selectedId={selection?.kind === 'idea' ? selection.id : null} onSelect={chooseIdea} /></Panel></aside>
        <section className="observatory-main"><Panel title="研究动态"><ActivityFeed activity={projection.activity} truncated={projection.activity_truncated} scanTruncated={projection.activity_scan_truncated} limit={projection.activity_limit} selectedId={selection?.kind === 'activity' ? selection.id : null} onSelect={chooseActivity} /></Panel></section>
        <aside className="observatory-inspector"><ExperimentActions runId={runId} projection={projection} onChanged={() => void loadProjection(runId, sessionId)} /><Panel title="详情面板"><DetailDrawer selection={detailSelection} onDiscuss={onDiscuss} /><AttemptList attempts={projection.attempts} selectedIdeaId={selection?.kind === 'idea' ? selection.id : null} selectedId={selection?.kind === 'attempt' ? selection.id : null} onSelect={chooseAttempt} /><DeveloperRefs projection={projection} show={showDeveloper} onToggle={() => setShowDeveloper(value => !value)} /></Panel></aside>
      </div>
    </>}
  </main>;
}
...
function ExperimentActions({ runId, projection, onChanged }: { runId: string; projection: ExperimentProjection; onChanged: () => void }) {
  ...
  return <Surface aria-label="实验确认动作" className="experiment-actions">
...
function SessionOverview({ projection }: { projection: ExperimentProjection }) {
  ...
  return <Surface className="session-overview">
    <div className="session-goal">{goal}</div>
    <div className="session-facts">
      <StatusBadge tone="info">Session：{sessionStatusLabel(projection.session?.status || '')}</StatusBadge><StatusBadge tone="neutral">环境：{environmentStatusLabel(projection.session?.environment_status || '')}</StatusBadge><StatusBadge tone="neutral">Baseline：{baselineStatusLabel(projection.session?.baseline_status || '')}</StatusBadge><StatusBadge tone="neutral">Idea：{projection.summary?.idea_count}</StatusBadge><StatusBadge tone="neutral">Champion：{champion}</StatusBadge>
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <Surface className="observatory-panel"><h2>{title}</h2>{children}</Surface>; }

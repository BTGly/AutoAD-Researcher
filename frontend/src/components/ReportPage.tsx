...
export function ReportPage({ runId, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([getReportState(runId, selectedId), getReportDigest(runId, selectedId), getReportContent(runId, selectedId), listReportEvidence(runId, selectedId), getReportDiscussion(runId, selectedId)])
      .then(([nextState, nextDigest, nextContent, nextEvidence, nextDiscussion]) => { setState(nextState); setDigest(nextDigest); setContent(nextContent); setEvidence(nextEvidence); setDiscussion(nextDiscussion.messages); })
  ...
  const sendDiscussion = async () => { if (!selected || !question.trim() || sending) return; setSending(true); try { await sendReportDiscussion(runId, selected.report_id, `report.${selected.report_id}.${crypto.randomUUID()}`, question.trim()); setQuestion(''); setDiscussion((await getReportDiscussion(runId, selected.report_id)).messages); } catch (reason) { setError(reason instanceof Error ? reason.message : '讨论请求失败'); } finally { setSending(false); } };

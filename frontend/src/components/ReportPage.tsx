...
import { confirmReportProposal, createHumanProposal, createReport, getExperimentProjection, getLatestContentReadyReport, getLatestCreatedReport, getReportContent, getReportDigest, getReportDiscussion, getReportState, listReportEvidence, listReportProposals, listReports, recordReportReview, rejectReportProposal, sendReportDiscussion } from '../lib/api';
  ...
  return <main className="report-workspace" aria-busy={loading || Boolean(selectedId && !visibleState)}>
    ...
    <div className="report-layout">
      ...
      <aside className="report-inspector">
        ...
        <section className="report-section report-evidence"><h2>证据</h2>{visibleEvidence.map(item => <details id={`evidence-${item.evidence_id}`} key={item.evidence_id}><summary>{reportEvidenceKindLabel(item.evidence_kind)} · {item.evidence_id}</summary><div>{item.summary}</div>{item.attempt_id && <div>实验轮次：{item.attempt_id}</div>}{item.idea_id && <div>研究想法：{item.idea_id}</div>}<div>制品类型：{reportEvidenceKindLabel(item.artifact_ref.artifact_type)}</div><div>大小：{formatBytes(item.artifact_ref.size_bytes)}</div><div>来源：{item.artifact_ref.source_id || item.source_object_id}</div><div>字段：{item.field_path}</div><div className="report-break">技术引用：{item.artifact_ref.locator}</div><div className="report-break">SHA：{item.artifact_ref.sha256}</div></details>)}</section>

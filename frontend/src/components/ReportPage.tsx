...
import { getLatestContentReadyReport, getLatestCreatedReport, getReportContent, getReportDigest, getReportState, listReportEvidence, listReports } from '../lib/api';
import type { ReportDigest, ReportEvidence, ReportManifest, ReportState } from '../lib/types';
...
export function ReportPage({ runId, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([getReportState(runId, selectedId), getReportDigest(runId, selectedId), getReportContent(runId, selectedId), listReportEvidence(runId, selectedId)])
      .then(([nextState, nextDigest, nextContent, nextEvidence]) => { setState(nextState); setDigest(nextDigest); setContent(nextContent); setEvidence(nextEvidence); })
      .catch(reason => setError(reason instanceof Error ? reason.message : '无法读取固定版本报告'));
  }, [runId, selectedId]);
  ...
  return <main style={{ flex: 1, overflow: 'auto', padding: '20px 28px', color: 'var(--text)' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(250px, .42fr)', gap: 18, marginTop: 16 }}>
      ...
      <aside style={{ borderLeft: '1px solid var(--border)', paddingLeft: 16 }}><h2 style={{ fontSize: '.95rem', marginTop: 0 }}>摘要</h2>{digest ? <div style={{ fontSize: '.84rem', lineHeight: 1.6 }}><div>执行状态：{digest.execution_status ?? '未记录'}</div><div>Attempts：{digest.attempt_count}，失败：{digest.failed_attempt_count}，不可比：{digest.non_comparable_attempt_count}</div>{digest.uncertainties.map(item => <div key={item} style={{ color: 'var(--text-muted)', marginTop: 6 }}>{item}</div>)}</div> : <div style={{ color: 'var(--text-muted)' }}>摘要尚不可用。</div>}<h2 style={{ fontSize: '.95rem', marginTop: 20 }}>证据</h2>{evidence.map(item => <details key={item.evidence_id} style={{ borderTop: '1px solid var(--border)', padding: '8px 0', fontSize: '.8rem' }}><summary>{item.evidence_kind} · {item.evidence_id}</summary><div style={{ marginTop: 5, color: 'var(--text-muted)' }}>{item.summary}</div>{item.attempt_id && <div>Attempt：{item.attempt_id}</div>}{item.idea_id && <div>Idea：{item.idea_id}</div>}<div style={{ overflowWrap: 'anywhere' }}>SHA：{item.artifact_ref.sha256}</div></details>)}</aside>
    </div></>}
  </main>;
}

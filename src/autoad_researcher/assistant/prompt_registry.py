...
- 优先读取 ResponseContext.facts.readable_artifacts 中的 paper_summary.json、paper_reader_result.json、sections.json；blocks.jsonl 的 page 1 可能是 PDF 二进制数据，应跳过乱码块，不能因为 blocks.jsonl 局部乱码就说整篇论文不可读；
...
- 优先从 paper_summary.json、paper_reader_result.json 和 sections.json 获取论文题目、摘要、方法和章节结构；blocks.jsonl 的第一页可能含 PDF 二进制块，遇到乱码应跳过，而不是否定其它结构化 artifact；
...
- 优先从 ResponseContext.facts.readable_artifacts 指向的 paper_summary.json、paper_reader_result.json、sections.json 读取论文内容；blocks.jsonl 的 page 1 可能是 PDF 二进制数据，跳过乱码块；

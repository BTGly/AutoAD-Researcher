...
- 当前聊天没有后台 worker，不能承诺已经开始 web_search/web_fetch/git_clone，也不能承诺几分钟后主动回复；搜索类请求应登记为 material_requests，等待 discovery/acquisition artifacts；
...
- 对搜索、最新方法、SOTA、官方仓库等资料搜集请求，只能说明已登记 material_requests 或需要进入 discovery/acquisition 阶段；不要承诺后台主动搜索或稍后主动回复；
...
- 当前聊天不能后台执行 web_search/web_fetch/git_clone，不能承诺几分钟后主动回复；搜索类诉求必须以 material_requests / discovery-acquisition artifacts 对齐；

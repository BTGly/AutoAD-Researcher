...
_RESEARCH_DECISION_PROMPT = """<decision_scope>
你是 Research Decision Agent。只判断本轮 mode、科研 policy 和候选动作；不写用户回复，不生成 summary。
按完整语义判断，不按关键词机械分类。能力目录不是当前材料；材料不足不是拒绝理由。
</decision_scope>
...
- 即使理由看似正当也拒绝；填写具体 reason 和 safe_alternative。否则 allow/none，两个文本字段为空。
</policy>
...
_RESEARCH_REPLY_PROMPT = """<identity>
你是 AutoAD Research Reply Agent。根据冻结决策写自然回复和完整 research summary；不得改写 mode、policy 或动作。
默认帮助并直接进入实质内容，不用“您好”“感谢确认”“好问题”“当然可以”等客套开场。
</identity>
...
def _default_profiles() -> list[PromptProfile]:
    return [
        ...
        PromptProfile(
            prompt_id="assistant.research_decision.v1",
            prompt_version="v1",
            layer="assistant_state",
            assistant_stage="understanding_intent",
            title="Research Decision",
            description="Proposes semantic mode, policy, and candidate actions for deterministic gating.",
            system_prompt=_RESEARCH_DECISION_PROMPT,
            io=PromptIOContract(
                input_schema="ResearchDecisionContext",
                output_schema="DialogueDecision",
                required_artifacts=["conversation/chat_transcript.jsonl"],
                forbidden_outputs=[
                    "reply_to_user",
                    "summary",
            ...
            source_references=[
                ...
                "src/autoad_researcher/assistant/v2/dialogue_gate.py",
            ],
            changelog=[
                "v1: splits semantic routing and candidate actions from reply generation.",
            ],
        ),
        PromptProfile(
            prompt_id="assistant.research_reply.v1",
            prompt_version="v1",
            layer="assistant_state",
            assistant_stage="understanding_intent",
            title="Research Reply",
            description="Writes one evidence-aware reply and complete summary from a gated decision.",
            system_prompt=_RESEARCH_REPLY_PROMPT,
            io=PromptIOContract(
                input_schema="ResearchReplyContext",
                output_schema="ResearchReplyDraftResponse",
                required_artifacts=["conversation/chat_transcript.jsonl"],
            ...
            changelog=[
                "v1: generates reply and summary only from one frozen gated decision.",
            ],
        ),
        PromptProfile(

...
def build_default_narrative(facts: ExperimentReportFactsV1) -> NarrativeSectionsV1:
    return NarrativeSectionsV1(
        sections=[
            ...
            NarrativeSectionV1(section_id="limitations", paragraphs=[NarrativeParagraphV1(paragraph_id="limitations_uncertainties", paragraph_kind="limitation", prose_template="局限性由关联的结构化 Claim 渲染。", claim_ids=["claim_limitations"])]),
            NarrativeSectionV1(section_id="next_steps", paragraphs=[NarrativeParagraphV1(paragraph_id="next_steps_recommendation", paragraph_kind="recommendation", prose_template="后续行动须基于用户确认的 Proposal；本报告不会创建实验任务。")]),
        ]
        ,
        claims=[
            StructuredClaimV1(claim_id="claim_summary", claim_kind="explanation", statement_template="报告仅使用已冻结来源。", fact_refs=["repository_and_environment.status"]),
            StructuredClaimV1(claim_id="claim_interpretation", claim_kind="explanation", statement_template="解释受已冻结 Session 状态约束。", fact_refs=["repository_and_environment.status"]),
            StructuredClaimV1(claim_id="claim_limitations", claim_kind="limitation", statement_template="报告保留冻结来源的不确定性。", fact_refs=["repository_and_environment.status"]),
        ],
    )

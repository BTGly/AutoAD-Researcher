...
from autoad_researcher.experiment.executor_agent import ExecutorAgent, ExecutorLimits, ExecutorProposal
...
class ExecutorAttemptHandoffService:
    def __init__(self, *, attempt_service: ExperimentAttemptService | None = None, adapter: ExecutorAdapter | None = None):
        self._attempts = attempt_service or ExperimentAttemptService(); self._adapter = adapter or ExecutorAdapter()
    def handoff(self, run_dir: Path, *, request: ExecutorHandoffRequest, proposal_provider: Callable) -> ExecutorHandoffResult:
        ...
        summary = ExecutorAgent(contract=request.intervention_contract, workspace=workspace, artifact_dir=staging, limits=ExecutorLimits(max_steps=8, max_wall_seconds=request.intervention_contract.time_budget, max_model_calls=request.intervention_contract.max_repairs + 1)).run(proposal_provider)
        ...
        for name in ["patch.diff", "repair_log.jsonl", "executor_summary.json"]:
            source = staging / name

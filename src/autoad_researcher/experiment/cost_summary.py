"""Derived usage summaries over the append-only cognitive ledger."""
...
class CognitiveCostSummaryBuilder:
    def __init__(self, *, store: CognitiveUsageStore | None = None):
        self._store = store or CognitiveUsageStore()
    ...
    def build(
        ...
        *,
        ...
        return _summary(session_id, usage, usage_sha256)
    ...
    def build_and_persist(
        ...
        *,
        ...
        with self._store._lock(run_dir, session_id):
            usage = self._store.load(run_dir, session_id=session_id)
            summary = _summary(session_id, usage, _usage_sha256(run_dir, session_id))
            path = run_dir / "experiments" / "cognition" / session_id / "cost_summary.json"
            _write_json_atomic(path, summary.model_dump(mode="json"))
        return summary


def _summary(session_id: str, usage, usage_sha256: str) -> CognitiveCostSummary:
    calls = len(usage)
...
def _usage_sha256(run_dir: Path, session_id: str) -> str:
    path = CognitiveUsageStore._path(run_dir, session_id)
